...
| **`ui/draw`** | A software rasterizer over premultiplied RGBA pixels: fills, anti-aliased rounded corners, borders, gradients, 8-bit masks, images resampled up or down, PNG encoding. | none |
...
| **`ui/image`** | Image decoding: PNG, JPEG, GIF, WebP and whatever the platform reads; `data:` URLs. | `cimage` (ImageIO) |
