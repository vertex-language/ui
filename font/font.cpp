// The platform's fonts, for package ui/font: font_darwin.mm (CoreText).
//
...
export int32_t fontFace(const char* family, double size, int32_t weight, int32_t italic) noexcept;
...
export int32_t fontShape(int32_t face, const char* text, int32_t len, uint32_t* glyphs,
                         int32_t* faces, float* advances, int32_t cap) noexcept;
...
export int32_t fontGlyph(int32_t face, uint32_t glyph, double scale, int32_t* left, int32_t* top,
                         int32_t* width, int32_t* height, uint8_t* buf, int32_t cap) noexcept;
